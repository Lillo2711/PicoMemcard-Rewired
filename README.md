...
it simply aim to take alberonisse project (Button + WS2812 Edition) and repinned it to comply with the original pin set from https://github.com/dangiu/PicoMemcard (or simply dangiu's original project) , cause i have already my Pico mmc Board and i don't want to trash it cause alberonisse like other pin instead the original.
Before doing anything stupid check the wiring of your project if it comply with Dangiu's original schematic ,then remember that the controller combination is removed and adde a phisical button on pin 3 and pin Gnd for Raspberry pi pico , and pin 4 and pin Gnd for Rp2040.
