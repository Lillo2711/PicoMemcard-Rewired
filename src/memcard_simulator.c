...
static uint8_t mc_data[MC_SIZE];
 ...
 */
void simulate_mc_reconnect(uint8_t posMC) {
...
bool is_mc_switch_safe() {
	return current_state == MC_IDLE && queue_is_empty(&mc_sector_sync_queue);
}
...
void process_config_button() {
    ...
    if (!pressed && button_prev) {
        ...
        if (held_ms > 20 && held_ms < 1000) {
            uint8_t next = imagePosition + 1;
            if (next >= imageCount) next = 0;

            imageToPosition = next;
...
_Noreturn int simulate_memory_card() {
	...
	char mc_file_name[MAX_MC_FILENAME_LEN];
    

	// Verifica SD
	sd_card_t *p_sd = sd_get_by_num(0);
    while(FR_OK != f_mount(&p_sd->fatfs, "", 1)){
        //SD nao responde ou SD sem FAT valido
        led_blink_error(MM_NO_SD_CARD);
    }

    //Busca o memory card no SD
	int8_t status = memcard_manager_restart(mc_file_name);
	if(status!=MM_OK) {
        while(true) {
	...
	while(true) {
        ...
        if(is_mc_switch_safe() && request_next_mc) {
			mutex_enter_blocking(&mutex_sm_tick);
            led_set_outpu(true);
            //----------------------------------------
            request_next_mc = false;
			char new_file_name[MAX_MC_FILENAME_LEN];
            int8_t status = memcard_manager_get_position(imageToPosition, new_file_name);
            imageToPosition = 0;
			if(status != MM_OK) {
                while(true){
                    led_blink_error(status);
                ...
                memset(mc_file_name, 0, MAX_MC_FILENAME_LEN);
                memcpy(mc_file_name, new_file_name, MAX_MC_FILENAME_LEN-1);
                mc_file_name[MAX_MC_FILENAME_LEN - 1] = '\0';
                saveSelectMC(mc_file_name);
				status = memory_card_import(&mc, mc_file_name);
				if(status != MC_OK){
                    while(true){
                        led_blink_error(status);
				...
				simulate_mc_reconnect(imagePosition+1);
            }
            //----------------------------------------
            led_set_outpu(false);
			mutex_exit(&mutex_sm_tick);
		...
		if(is_mc_switch_safe() && request_new_mc) {
			mutex_enter_blocking(&mutex_sm_tick);
            led_output_newMC();
            led_set_outpu(true);
            //----------------------------------------
